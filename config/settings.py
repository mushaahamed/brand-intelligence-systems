...
# Fallback: if a group has no token, use TOKEN_1
_single_token = os.getenv("APIFY_TOKEN_1", "")
for key in APIFY_TOKENS:
    if not APIFY_TOKENS[key]:
        APIFY_TOKENS[key] = _single_token

# All unique tokens for fallback rotation (tried in order when primary fails)
_seen: set = set()
APIFY_ALL_TOKENS: list = []
for _t in [
    ...
    if _t and _t not in _seen:
        _seen.add(_t)
        APIFY_ALL_TOKENS.append(_t)

# ─── LLM (OpenAI) ─────────────────────────────────────────────────────────────
OPENAI_API_KEY    = os.getenv("OPENAI_API_KEY", "")
OPENAI_MODEL      = "gpt-4o-mini"   # fast + cheap for all synthesis
OPENAI_MODEL_FULL = "gpt-4o"        # used only for outreach writing
